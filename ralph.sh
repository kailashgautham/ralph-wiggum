...
if [ "${1:-}" = "--help" ] || [ "${1:-}" = "-h" ]; then
  cat <<'EOF'
Usage: ./ralph.sh [--help|-h] [status] [--dry-run] [max_iterations]

Subcommands:
  status            Show task completion summary
...
Examples:
  ...
  ./ralph.sh status        # Show completion status
  ./ralph.sh --dry-run     # Show next task without running
EOF
  exit 0
fi
